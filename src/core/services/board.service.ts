...
export class BoardService {
    ...
    getNeighboringCoordinatesList(board: Board, coordinates: Coordinates): Coordinates[] {
    ...
    getTileChain(board: Board, coordinates: Coordinates): TileChain | undefined {
        return board.tileChains.find(tileChain => tileChain.coordinatesList.includes(coordinates));
    }

    getNeighboringTileChains(board: Board, coordinates: Coordinates): TileChain[] {
        const neighboringTileChains: TileChain[] = [];

        const neighboringCoordinatesList = this.getNeighboringCoordinatesList(board, coordinates);

        neighboringCoordinatesList.forEach((neighboringCoordinates) => {
            const tileChain = this.getTileChain(board, neighboringCoordinates);
    ...
    getCoordinatesCardEffect(board: Board, coordinates: Coordinates): CoordinatesCardEffect {
        const neighboringTileChains = this.getNeighboringTileChains(board, coordinates);
